...
- Stage Q5 — safe embedded Monaco F4 editor: **Done**.
  ...
  - Stage verification: targeted editor/asset/F4 suite 16/16; full checked-out suite 224/224; stability scenarios 14/14 in three consecutive runs; Debug and Release builds 0 warnings/errors; npm audit found no issue down to low severity and a clean locked rebuild reproduced the assets; NuGet audit found no vulnerable direct or transitive package; self-contained Linux and Windows publishes contained 11 manifest-verified Monaco files; `git diff --check` passed. A browser-engine smoke test rendered Monaco without console warnings. Hosted Linux/Windows CI run `33271748556` completed successfully, including the Monaco reproducibility gate and repeated stability scenarios. This local Linux host lacks an Avalonia-supported WPE/WebKit runtime, so native embedded-WebView interaction remains an explicit real-platform acceptance limitation.
