...
The two panels now include independent persistent tabs and a persistent editable favorite-directory hotlist. Every tab keeps its own target, path, back/forward history and filter state; unavailable restored paths stay visible but are never opened outside their validated indexed target. Per-panel, case-configurable contains/glob/regex quick filters are evaluated over the complete cached directory snapshot before the 400-row UI page is produced, so matches beyond the first page remain discoverable. Commander selection commands (all, invert, same extension, mask, restore previous), tab commands, and F2/F3–F8 shortcuts always resolve the active panel. See [Keyboard shortcuts](docs/keyboard-shortcuts.md). F3 opens bounded Quick View for one local file; double-click or the context-menu Open action navigates directories and archives or delegates ordinary files to the operating system.

Quick View reads only a selectable 16–256 KiB block rather than loading a large file into memory. It auto-detects BOM-based UTF-8/UTF-16 and binary content, allows an explicit supported encoding, and can show every local regular file as an offset-addressed hex dump. Page Up/Down and first/last commands navigate blocks. The source length and modification time are bound to the session and revalidated around every read; links and linked ancestors are rejected. Loading is cancellable and the viewer captures commander keys while open. Syntax highlighting, images/EXIF, PDF/media/document presentation, archive entries and SFTP sources remain planned; see [Quick View](docs/quick-view.md).
...
Official `win-x64` and `linux-x64` release archives carry a verified, replaceable 7-Zip 26.02 native library, so read-only browsing/extraction is additionally advertised for 7z, TAR.GZ, GZip, BZip2, XZ and RAR when that library loads successfully. Plain TAR uses the managed .NET reader/writer and therefore does not depend on the native engine. Creating 7z or compressed TAR variants, modifying non-ZIP archives, and previewing an unextracted entry are not yet implemented; see [the Commander modernization roadmap](docs/commander-roadmap.md).
