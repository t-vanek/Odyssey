...
Quick View keeps memory bounded to a selectable 16–256 KiB block rather than loading a large file into RAM. It auto-detects BOM-based UTF-8/UTF-16 and binary content, allows an explicit supported encoding, and shows accepted local, archive and SFTP files as offset-addressed hex dumps. Archive entries are validated with the full hostile-catalogue policy and streamed without an extracted or persistent cache file; reaching a later block in compressed data can still require decoding and discarding its prefix. Connected SFTP files use cancellable ranged reads over the already host-key-verified session and never receive a password or create a local preview file. Page Up/Down and first/last commands navigate blocks. The local source, archive/entry, or remote length/time version is bound to the viewer and revalidated around reads; links are rejected. Loading is cancellable and the viewer captures commander keys while open. Syntax highlighting and image/EXIF/PDF/media/document presentation remain planned; see [Quick View](docs/quick-view.md).
